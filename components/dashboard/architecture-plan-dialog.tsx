...
export function ArchitecturePlanDialog({ plan, open, onOpenChange }: ArchitecturePlanDialogProps) {
  ...
  const handleSubmitFeedback = async () => {
    ...
    if (!swarmClient) {
      toast.error("Swarm connection not ready")
      return
    }
    ...
    try {
      const updatedRequirements = {
        ...(projectRequirements || {}),
        notes: [...((projectRequirements && projectRequirements.notes) || []), `Feedback: ${feedback}`],
      }
      setProjectRequirements(updatedRequirements)
      swarmClient.updateBrief(updatedRequirements)
      setPlanningStatus("in_progress")
      setIsDebating(true)
      setDebateFinished(false)
      setSwarmDocMarkdown(null)
      await swarmClient.startDebate(`${projectBrief}\n\nFeedback for revision: ${feedback}`, agents)
      toast.success("Feedback submitted! Agents are refining the plan.")
      setFeedback("")
    } catch (error) {
  ...
  const handleApprovePlan = () => {
    ...
    if (sessionId) {
      fetch(`${BACKEND_URL}/projects/plan/export`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ session_id: sessionId }),
      }).catch((error) => console.error("export-plan", error))
    }
    onOpenChange(false)
  }

  if (!plan && !swarmDocMarkdown) return null

  const projectName = plan?.projectName || "AI Swarm Execution Plan"
  const overview = plan?.overview || "Generated by the AI Swarm."
  const components = plan?.components || []
  const techStack = plan?.techStack || []
  const timeline = plan?.timeline || ""
  const docContent = swarmDocMarkdown || ""
  const hasDoc = Boolean(docContent)
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col bg-black/95 backdrop-blur-2xl border-primary/30 shadow-2xl shadow-primary/20">
        <DialogHeader>
          <DialogTitle className="text-2xl flex items-center gap-2 bg-gradient-to-r from-primary via-cyan-400 to-primary bg-clip-text text-transparent">
            <span className="text-2xl">🎯</span> {projectName}
          </DialogTitle>
        ...
        <ScrollArea className="flex-1 min-h-0">
          <div className="space-y-6 pr-4">
            <div className="p-4 rounded-lg bg-primary/5 border border-primary/20">
              {hasDoc ? (
                <ScrollArea className="max-h-[60vh] border border-primary/10 rounded-lg">
                  <pre className="whitespace-pre-wrap text-sm p-4 bg-black/60 text-muted-foreground">
                    {docContent}
                  </pre>
                </ScrollArea>
              ) : (
        ...
        <div className="border-t border-primary/20 pt-4 space-y-3 bg-black/95">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Button
              ...
              className="flex-1 gap-2 bg-gradient-to-r from-primary to-cyan-500 hover:from-primary/90 hover:to-cyan-500/90 shadow-lg shadow-primary/50"
            >
              <Send className="w-4 h-4" />
              {isSubmitting ? "Submitting..." : "Request Replan"}
            </Button>
            <Button
              type="button"
